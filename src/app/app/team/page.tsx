...
// Use different names + no nulls
type ServerMember = { userId: string; role: "owner"|"admin"|"member"; email?: string; name?: string };
type ServerInvite = { email: string; role: "owner"|"admin"|"member"; token: string; expiresAt?: Date };

type MemberDoc = { orgId: string; userId: string; role: "owner"|"admin"|"member" };
type InviteDoc = ServerInvite & { orgId: string; status: "pending"|"accepted"|"revoked" };
...
export default async function TeamPage() {
    ...
    const userMap = new Map(
        userDocs.map((u: any) => [
            String(u._id),
            {
                email: (u.email as string | undefined)?.toLowerCase(), // string | undefined
                name: u.name as string | undefined,
            },
        ])
    );

    // <<< return undefined (not null) when missing
    const members: ServerMember[] = memberDocs.map(m => ({
        userId: m.userId,
        role: m.role,
    ...
    const inviteDocs = await invitesCol
        .find(
            ...
            { projection: { _id: 0, email: 1, role: 1, token: 1, expiresAt: 1 } }
        )
        .sort({ createdAt: -1 })
        .toArray();

    const invites: ServerInvite[] = inviteDocs.map(({ email, role, token, expiresAt }) => ({
    ...
    // OK: client props accept email?: string; name?: string
